/* 
Realizar una función que reciba dos callbacks. El primer callback devuelve el array de objetos que se deben unir luego del tiempo en segundos que indica el retorno del segundo callback. 
...
function objectMerge(array, tiempo) {
  ...
  if (Array.isArray(array())) {
    if (typeof (tiempo()) != 'number' || tiempo() <= 0) {
      throw 'Tiempo de entrada no válido'
    }
    let mergeObj = {};
    return new Promise(function (resolve) {
      array().map(obj => mergeObj = {
        ...mergeObj,
        ...obj
      });
      setTimeout(function () {
    ...
    throw 'Array de entrada no válido'
  }
}
...
si la invocas de nuevo con 5, deberia retornar 25 (guardado previamente en el cache).
